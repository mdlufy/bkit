...
def catalog(message):
    ...
    features = telebot.types.KeyboardButton(text="Список пользователей")
    catalogKBoard.add(get_info, features)
    bot.send_message(message.chat.id, "Выберите Раздел", reply_markup=catalogKBoard)

# Показать список пользователей
@bot.message_handler(func=lambda message: message.text.lower() == 'список пользователей')
...
def cmd_reset(message):
    bot.send_message(message.chat.id, "Что ж, начнём по-новой. Как вас зовут?")
    dbworker.set_state(message.chat.id, config.States.S_ENTER_NAME.value)
